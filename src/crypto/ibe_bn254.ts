 ...
 * with the identity on G1, and the master public key on G2.
 */
export function decrypt_towards_identity_g1(ciphertext: Ciphertext, decryption_key_g1: G1, opts: IbeOpts = DEFAULT_OPTS): Uint8Array {
    ...
    const sigma2 = xor(ciphertext.V, hash_shared_key_to_bytes(shared_key, ciphertext.V.length, opts))
...
function hash_sigma_to_bytes(sigma: Uint8Array, n: number, opts: IbeOpts): Uint8Array {
    if (opts.expand_fn == "xmd") {
        return expand_message_xmd(sigma, opts.dsts.H2, n, opts.hash)
    } else {
        return expand_message_xof(sigma, opts.dsts.H2, n, opts.k, opts.hash)
    }
}
